...
>(({ className, children, disableFocusRing = false, ...props }, ref) => (
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <span className="relative ml-2 h-4 w-4" data-state-icon>
        <ChevronDown className="radix-state-open:hidden h-4 w-4" />
        <ChevronUp className="radix-state-open:block h-4 w-4" />
      </span>
  ...
  <SelectPrimitive.Portal>
    ...
    >
      ...
      <SelectPrimitive.Viewport
        className={cn(
          "max-h-[min(20rem,var(--radix-select-content-available-height))] overflow-y-auto overscroll-contain p-1",
          position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "focus:bg-accent focus:text-accent-foreground relative flex w-full items-center rounded-xs py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50",
      className,
    )}
    {...props}
  >
